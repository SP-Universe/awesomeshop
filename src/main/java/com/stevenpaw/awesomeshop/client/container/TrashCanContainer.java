...
public class TrashCanContainer extends Container {
    ...
    int startChestX = 80;                                //The pixelX on which the first Slot starts
    int startChestY = 16;                               //The pixelY on which the first Slot starts
    ...
    int columnCount = 1;                                //Number of Columns for the Chest

    int playerInventoryStartX = 8;                      //The pixelX on which the first Slot of the PlayerInventory starts
    int inventorySpacing = 52;                          //The space between the Container and the PlayerInventory
    ...
    private static TrashCanTileEntity getTileEntity(final PlayerInventory playerInventory, final PacketBuffer data) {
        ...
        if (tileAtPos instanceof TrashCanTileEntity) {
            return (TrashCanTileEntity) tileAtPos;
        }
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(canInteractWithCallable, playerIn, BlockInit.TRASHCAN.get());
    }
