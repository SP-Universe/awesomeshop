...
public class SignPainterContainer extends Container {

    private final IWorldPosCallable worldPosCallable;
    /** The index of the selected recipe in the GUI. */
    private final IntReferenceHolder selectedRecipe = IntReferenceHolder.single();
    private final World world;
    private List<SignPainterRecipe> recipes = Lists.newArrayList();
    ...
    public final IInventory inputInventory = new Inventory(1) {
        ...
        public void markDirty() {
            super.markDirty();
            SignPainterContainer.this.onCraftMatrixChanged(this);
            SignPainterContainer.this.inventoryUpdateListener.run();
        }
    };
    /** The inventory that stores the output of the crafting recipe. */
    private final CraftResultInventory inventory = new CraftResultInventory();

    public SignPainterContainer(int windowIdIn, PlayerInventory playerInventoryIn, PacketBuffer packetBuffer) {
        this(windowIdIn, playerInventoryIn, IWorldPosCallable.DUMMY);
    }

    public SignPainterContainer(int windowIdIn, PlayerInventory playerInventoryIn, final IWorldPosCallable worldPosCallableIn) {
        super(ModContainerTypes.SIGN_PAINTER.get(), windowIdIn);
        ...
        this.outputInventorySlot = this.addSlot(new Slot(this.inventory, 1, 143, 33) {
            ...
            public ItemStack onTake(PlayerEntity thePlayer, ItemStack stack) {
                ...
                SignPainterContainer.this.inventory.onCrafting(thePlayer);
                ItemStack itemstack = SignPainterContainer.this.inputInventorySlot.decrStackSize(1);
                if (!itemstack.isEmpty()) {
                    SignPainterContainer.this.updateRecipeResultSlot();
                }

                worldPosCallableIn.consume((p_216954_1_, p_216954_2_) -> {
                    long l = p_216954_1_.getGameTime();
                    if (SignPainterContainer.this.lastOnTake != l) {
                        ...
                        SignPainterContainer.this.lastOnTake = l;
                    }
    ...
    @OnlyIn(Dist.CLIENT)
    public List<SignPainterRecipe> getRecipeList() {
        return this.recipes;
    }
    ...
    private void updateAvailableRecipes(IInventory inventoryIn, ItemStack stack) {
        ...
        if (!stack.isEmpty()) {
            this.recipes = this.world.getRecipeManager().getRecipes((IRecipeType) ModRecipeType.SIGN_PAINTER, inventoryIn, this.world);
        }
    ...
    private void updateRecipeResultSlot() {
        if (!this.recipes.isEmpty() && this.func_241818_d_(this.selectedRecipe.get())) {
            SignPainterRecipe signPainterRecipe = this.recipes.get(this.selectedRecipe.get());
            this.inventory.setRecipeUsed(signPainterRecipe);
            this.outputInventorySlot.putStack(signPainterRecipe.getCraftingResult(this.inputInventory));
        } else {
            this.outputInventorySlot.putStack(ItemStack.EMPTY);
        }
    ...
    public ContainerType<?> getType() {
        return ModContainerTypes.SIGN_PAINTER.get();
    }
    ...
    public ItemStack transferStackInSlot(PlayerEntity playerIn, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            } else if (this.world.getRecipeManager().getRecipe(ModRecipeType.SIGN_PAINTER, new Inventory(itemstack1), this.world).isPresent()) {
