...
import com.stevenpaw.awesomeshop.recipe.AwesomeShopRecipe;
...
public class AwesomeShopContainer extends Container {

    private final IWorldPosCallable worldPosCallable;
    /** The index of the selected recipe in the GUI. */
    private final IntReferenceHolder selectedRecipe = IntReferenceHolder.single();
    private final World world;
    private List<AwesomeShopRecipe> recipes = Lists.newArrayList();
    ...
    public final IInventory inputInventory = new Inventory(1) {
        ...
        public void markDirty() {
            super.markDirty();
            AwesomeShopContainer.this.onCraftMatrixChanged(this);
            AwesomeShopContainer.this.inventoryUpdateListener.run();
        }
    };
    /** The inventory that stores the output of the crafting recipe. */
    private final CraftResultInventory inventory = new CraftResultInventory();

    public AwesomeShopContainer(int windowIdIn, PlayerInventory playerInventoryIn, PacketBuffer packetBuffer) {
        this(windowIdIn, playerInventoryIn, IWorldPosCallable.DUMMY);
    }

    public AwesomeShopContainer(int windowIdIn, PlayerInventory playerInventoryIn, final IWorldPosCallable worldPosCallableIn) {
        super(ModContainerTypes.AWESOMESHOP.get(), windowIdIn);
        ...
        this.outputInventorySlot = this.addSlot(new Slot(this.inventory, 1, 143, 33) {
            ...
            public ItemStack onTake(PlayerEntity thePlayer, ItemStack stack) {
                ...
                AwesomeShopContainer.this.inventory.onCrafting(thePlayer);
                ItemStack itemstack = AwesomeShopContainer.this.inputInventorySlot.decrStackSize(1);
                if (!itemstack.isEmpty()) {
                    AwesomeShopContainer.this.updateRecipeResultSlot();
                }

                worldPosCallableIn.consume((p_216954_1_, p_216954_2_) -> {
                    long l = p_216954_1_.getGameTime();
                    if (AwesomeShopContainer.this.lastOnTake != l) {
                        ...
                        AwesomeShopContainer.this.lastOnTake = l;
                    }
    ...
    @OnlyIn(Dist.CLIENT)
    public List<AwesomeShopRecipe> getRecipeList() {
        return this.recipes;
    }
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(this.worldPosCallable, playerIn, BlockInit.AWESOMESHOP.get());
    }
    ...
    private void updateAvailableRecipes(IInventory inventoryIn, ItemStack stack) {
        ...
        if (!stack.isEmpty()) {
            this.recipes = this.world.getRecipeManager().getRecipes((IRecipeType) ModRecipeType.AWESOMESHOP, inventoryIn, this.world);
        }
    ...
    private void updateRecipeResultSlot() {
        if (!this.recipes.isEmpty() && this.func_241818_d_(this.selectedRecipe.get())) {
            AwesomeShopRecipe awesomeshoprecipe = this.recipes.get(this.selectedRecipe.get());
            this.inventory.setRecipeUsed(awesomeshoprecipe);
            this.outputInventorySlot.putStack(awesomeshoprecipe.getCraftingResult(this.inputInventory));
        } else {
            this.outputInventorySlot.putStack(ItemStack.EMPTY);
        }
    ...
    public ContainerType<?> getType() {
        return ModContainerTypes.AWESOMESHOP.get();
    }
    ...
    public ItemStack transferStackInSlot(PlayerEntity playerIn, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            } else if (this.world.getRecipeManager().getRecipe(ModRecipeType.AWESOMESHOP, new Inventory(itemstack1), this.world).isPresent()) {
