...
public class ShredderTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    public ShredderTileEntity(TileEntityType<?> tileEntityTypeIn) {
        ...
        this.inventory = new ShredderItemHandler(2);
    }

    public ShredderTileEntity() {
        this(ModTileEntityTypes.SHREDDER.get());
    }

    @Override
    public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
        return new ShredderContainer(windowID, playerInv, this);
    }
    ...
    private ITextComponent getDefaultName() {
        return new TranslationTextComponent("container." + AwesomeShop.MOD_ID + ".example_furnace");
    }
    ...
    @Nullable
    private ShredderRecipe getRecipe(ItemStack stack) {
        if (stack == null) {
            return null;
        }

        Set<IRecipe<?>> recipes = findRecipesByType(ModRecipeSerializers.SHREDDER_TYPE, this.world);
        for (IRecipe<?> iRecipe : recipes) {
            ShredderRecipe recipe = (ShredderRecipe) iRecipe;
