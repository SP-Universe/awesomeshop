...
import com.stevenpaw.awesomeshop.tileentity.TrashCanTileEntity;
...
public class TrashCan extends Block {
    ...
    {
        return this.getDefaultState().with(FACING, context.getPlacementHorizontalFacing().getOpposite()).with(OPEN, false);
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(!worldIn.isRemote) {
            ...
            if(tile instanceof TrashCanTileEntity){
                NetworkHooks.openGui((ServerPlayerEntity)player, (TrashCanTileEntity)tile, pos);
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if(state.getBlock() != newState.getBlock()) {
            ...
            if(te instanceof TrashCanTileEntity) {
                InventoryHelper.dropItems(worldIn, pos, ((TrashCanTileEntity)te).getItems());
            }
        }
    }
    ...
    public void addInformation(ItemStack stack, @Nullable IBlockReader world, List<ITextComponent> tooltip, ITooltipFlag flag)
    {
        if(InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            ...
            tooltip.add(new StringTextComponent(TextFormatting.BLUE + "A Trashcan to remove all"));
            tooltip.add(new StringTextComponent(TextFormatting.BLUE + "your unwanted things!"));
        }
        else
        {
