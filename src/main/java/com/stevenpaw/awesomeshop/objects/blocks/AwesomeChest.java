...
public class AwesomeChest extends Block {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(!worldIn.isRemote) {
            ...
            if(tile instanceof AwesomeChestTileEntity){
                NetworkHooks.openGui((ServerPlayerEntity)player, (AwesomeChestTileEntity)tile, pos);
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if(state.getBlock() != newState.getBlock()) {
            ...
            if(te instanceof AwesomeChestTileEntity) {
                InventoryHelper.dropItems(worldIn, pos, ((AwesomeChestTileEntity)te).getItems());
            }
        }
    }
    ...
    public void addInformation(ItemStack stack, @Nullable IBlockReader world, List<ITextComponent> tooltip, ITooltipFlag flag)
    {
        if(InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            ...
            tooltip.add(new StringTextComponent(TextFormatting.BLUE + "A large Chest with"));
            tooltip.add(new StringTextComponent(TextFormatting.BLUE + "36 Slots"));
        }
        else
        {
